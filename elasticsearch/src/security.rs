...
#[derive(Debug, Clone, PartialEq, Eq)]
#[doc = "API parts for the Security Clone Api Key API"]
pub enum SecurityCloneApiKeyParts {
    #[doc = "No parts"]
    None,
}
impl SecurityCloneApiKeyParts {
    #[doc = "Builds a relative URL path to the Security Clone Api Key API"]
    pub fn url(self) -> Cow<'static, str> {
        match self {
            SecurityCloneApiKeyParts::None => "/_security/api_key/clone".into(),
        }
    }
}
#[doc = "Builder for the [Security Clone Api Key API](https://www.elastic.co/docs/api/doc/elasticsearch/operation/operation-security-clone-api-key)\n\nClone an API key. Creates a new API key with the same role descriptors as an existing key, with a new name, id, and optional expiration and metadata."]
#[derive(Clone, Debug)]
pub struct SecurityCloneApiKey<'a, 'b, B> {
    transport: &'a Transport,
    parts: SecurityCloneApiKeyParts,
    body: Option<B>,
    error_trace: Option<bool>,
    filter_path: Option<&'b [&'b str]>,
    headers: HeaderMap,
...
impl<'a, 'b, B> SecurityCloneApiKey<'a, 'b, B>
where
    B: Body,
{
    #[doc = "Creates a new instance of [SecurityCloneApiKey]"]
    pub fn new(transport: &'a Transport) -> Self {
        let headers = HeaderMap::new();
        SecurityCloneApiKey {
            transport,
            parts: SecurityCloneApiKeyParts::None,
            headers,
            body: None,
            error_trace: None,
            filter_path: None,
    ...
    #[doc = "The body for the API call"]
    pub fn body<T>(self, body: T) -> SecurityCloneApiKey<'a, 'b, JsonBody<T>>
    where
        T: Serialize,
    {
        SecurityCloneApiKey {
    ...
    #[doc = "If `true` then refresh the affected shards to make this operation visible to search, if `wait_for` then wait for a refresh to make this operation visible to search, if `false` then do nothing with refreshes. Default is `wait_for` in stateful and `true` in serverless."]
    pub fn refresh(mut self, refresh: Refresh) -> Self {
        self.refresh = Some(refresh);
        self
    }
    ...
    #[doc = "Creates an asynchronous call to the Security Clone Api Key API that can be awaited"]
    pub async fn send(self) -> Result<Response, Error> {
        let path = self.parts.url();
        let method = http::Method::Post;
        let headers = self.headers;
...
impl<'a, 'b> SecurityGetRole<'a, 'b> {
    ...
    #[doc = "If true, include privileges that are implicitly granted by registered ImplicitPrivilegesProviders alongside the explicitly configured privileges. These implicit privileges are computed for each role individually; for a user holding multiple roles, the privileges effectively granted at authorization time may therefore differ from the per-role view returned here."]
    pub fn include_implicit(mut self, include_implicit: bool) -> Self {
        self.include_implicit = Some(include_implicit);
        self
    }
    #[doc = "Pretty format the returned JSON response."]
    pub fn pretty(mut self, pretty: bool) -> Self {
...
impl<'a> Security<'a> {
    ...
    #[doc = "[Security Clone Api Key API](https://www.elastic.co/docs/api/doc/elasticsearch/operation/operation-security-clone-api-key)\n\nClone an API key. Creates a new API key with the same role descriptors as an existing key, with a new name, id, and optional expiration and metadata."]
    pub fn clone_api_key<'b>(&'a self) -> SecurityCloneApiKey<'a, 'b, ()> {
        SecurityCloneApiKey::new(self.transport())
    }
