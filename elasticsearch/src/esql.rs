...
pub struct EsqlAsyncQuery<'a, 'b, B> {
    ...
    allow_partial_dsl_filter: Option<bool>,
    ...
    B: Body,
{
    ...
    #[doc = "If `true`, unsupported clauses in the request `filter` are silently dropped and the translatable subset is applied;\na `Warning` header lists the dropped constructs.\nIf `false` (default), any unsupported construct fails the query with a 400."]
    pub fn allow_partial_dsl_filter(mut self, allow_partial_dsl_filter: bool) -> Self {
        self.allow_partial_dsl_filter = Some(allow_partial_dsl_filter);
        self
    }
        ...
        T: Serialize,
    {
        EsqlAsyncQuery {
            ...
            parts: self.parts,
            body: Some(body.into()),
            allow_partial_dsl_filter: self.allow_partial_dsl_filter,
...
#[cfg(feature = "experimental-apis")]
#[derive(Debug, Clone, PartialEq, Eq)]
#[doc = "API parts for the Esql Get Query API"]
pub enum EsqlGetQueryParts<'b> {
    #[doc = "Id"]
    Id(&'b str),
}
#[cfg(feature = "experimental-apis")]
impl<'b> EsqlGetQueryParts<'b> {
    #[doc = "Builds a relative URL path to the Esql Get Query API"]
    pub fn url(self) -> Cow<'static, str> {
        match self {
            EsqlGetQueryParts::Id(id) => {
                let encoded_id: Cow<str> = percent_encode(id.as_bytes(), PARTS_ENCODED).into();
                let mut p = String::with_capacity(16usize + encoded_id.len());
                p.push_str("/_query/queries/");
                p.push_str(encoded_id.as_ref());
                p.into()
            }
        }
...
#[cfg(feature = "experimental-apis")]
#[derive(Clone, Debug)]
pub struct EsqlGetQuery<'a, 'b> {
    transport: &'a Transport,
    parts: EsqlGetQueryParts<'b>,
    error_trace: Option<bool>,
    filter_path: Option<&'b [&'b str]>,
    headers: HeaderMap,
    human: Option<bool>,
...
#[cfg(feature = "experimental-apis")]
impl<'a, 'b> EsqlGetQuery<'a, 'b> {
    #[doc = "Creates a new instance of [EsqlGetQuery] with the specified API parts"]
    pub fn new(transport: &'a Transport, parts: EsqlGetQueryParts<'b>) -> Self {
        let headers = HeaderMap::new();
        EsqlGetQuery {
            transport,
            parts,
    ...
    #[doc = "Creates an asynchronous call to the Esql Get Query API that can be awaited"]
    pub async fn send(self) -> Result<Response, Error> {
        let path = self.parts.url();
        let method = http::Method::Get;
        let headers = self.headers;
...
#[cfg(feature = "experimental-apis")]
impl EsqlListQueriesParts {
    #[doc = "Builds a relative URL path to the Esql List Queries API"]
    pub fn url(self) -> Cow<'static, str> {
        match self {
            EsqlListQueriesParts::None => "/_query/queries".into(),
        }
    }
}
...
#[cfg(feature = "experimental-apis")]
#[derive(Clone, Debug)]
pub struct EsqlListQueries<'a, 'b> {
    transport: &'a Transport,
    parts: EsqlListQueriesParts,
    error_trace: Option<bool>,
    filter_path: Option<&'b [&'b str]>,
    headers: HeaderMap,
    human: Option<bool>,
...
#[cfg(feature = "experimental-apis")]
impl<'a, 'b> EsqlListQueries<'a, 'b> {
    #[doc = "Creates a new instance of [EsqlListQueries]"]
    pub fn new(transport: &'a Transport) -> Self {
        let headers = HeaderMap::new();
        EsqlListQueries {
            transport,
            parts: EsqlListQueriesParts::None,
            headers,
            error_trace: None,
            filter_path: None,
            human: None,
    ...
    #[doc = "Creates an asynchronous call to the Esql List Queries API that can be awaited"]
    pub async fn send(self) -> Result<Response, Error> {
        let path = self.parts.url();
        let method = http::Method::Get;
        let headers = self.headers;
    ...
    B: Body,
{
    ...
    #[doc = "A short version of the Accept header, e.g. json, yaml.\n\n`csv`, `tsv`, `txt`, and `md` formats will return results in a tabular format, excluding other metadata fields from the response."]
    pub fn format(mut self, format: Format) -> Self {
        self.format = Some(format);
        self
    }
