...
impl<'a> YamlTests<'a> {
    ...
    pub fn build(self) -> Tokens {
        ...
        let general_setup_call = quote!(client::general_xpack_setup().await?;);
    ...
    for entry in paths.flatten() {
        if let Ok(file_type) = entry.file_type() {
            if file_type.is_dir() {
                ...
                let folder = match relative_path.to_str().unwrap().find('/') {
                    None => {
                ...
                let test_location = folder.join(relative_path);
                let mut test = YamlTests::new(&test_location, version, &skips, docs.len());
                ...
                let results : Vec<anyhow::Result<()>> = docs
                    ...
                    .map(|doc| {
                        ...
                        match (key, value) {
                            (Yaml::String(name), Yaml::Hash(requires)) => {
                                if name.as_str() != "requires" {
                                    Err(anyhow!(
                                        "expected \"requires\" key but found {:?}",
                                        &name,
                                    ))
                                }
                                else {
                                    match requires.get(&Yaml::String("stack".into())) {
                                        Some(Yaml::Boolean(true)) => Ok(()),
                                        _ => Err(anyhow!("stack is not supported"))
                                    }
                                }
                            }
                ...
                match ok_or_accumulate(&results) {
                    Ok(_) => write_test_file(test, &test_location, generated_dir)?,
                    Err(e) => {
